...
static void T_StrobeFlash(strobe_t* const pFlash) noexcept {
    ...
    if (pFlash->sector->lightlevel == pFlash->minlight) {   // Already dim?
        pFlash->sector->lightlevel = pFlash->maxlight;      // Make it bright
        pFlash->count = pFlash->brighttime;                 // Reset timer
    } else {
        pFlash->sector->lightlevel = pFlash->minlight;      // Make it dim
        pFlash->count = pFlash->darktime;                   // Reset timer
    }
}
...
void P_SpawnStrobeFlash(sector_t& sector, const uint32_t fastOrSlow, const bool bInSync) noexcept {
    strobe_t& flash = *(strobe_t*) AddThinker((ThinkerFunc) T_StrobeFlash, sizeof(strobe_t));
